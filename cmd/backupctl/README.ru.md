...
## Хранение — сколько хранятся бэкапы

По умолчанию backupctl хранит все снапшоты вечно — ничего не удаляется
автоматически. Добавь `retention:`, чтобы `--init-server` запускал
`restic forget --prune` после каждого бэкапа — будет оставаться самый
свежий снапшот за каждый день/неделю/месяц/год в пределах заданных чисел:

```yaml
retention:
  keep_daily: 90     # ежедневные снапшоты хранить 90 дней
  keep_monthly: 36   # ежемесячные — 3 года (36 месяцев)
  keep_yearly: 15    # ежегодные — 15 лет
```

Любое из полей `keep_daily`, `keep_weekly`, `keep_monthly`, `keep_yearly`
можно не указывать или оставить 0 — в `restic forget` попадут только
заданные. Если `retention:` вообще не указан (или все поля 0) — команда
`forget` не запускается, поведение как сейчас.
...
retention:            # опционально — см. "Хранение" выше, без него — хранить вечно
  keep_daily: 90
  keep_monthly: 36
  keep_yearly: 15
